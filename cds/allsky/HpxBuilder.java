...
*/
     ...
     * @return
     */
	Fits buildHealpix(int nside_file, long npix_file, int nside, boolean fast) {
	   boolean empty = true;
	   long min;
	   long index;
	   double point[] = new double[2];
...
//	      if (!askLocalFinder(localServer, npix_file, (int) CDSHealpix.log2(nside)))
	      if (!askLocalFinder(downFiles,localServer, npix_file, Util.order(nside)))
	         return null;
	         ...
	         for (int x = 0; x < out.width; x++) {
	            ...
	            if (fast) {
	            	...
	            	for( int i=downFiles.size()-1; i>=0 && nbPix<pixval.length; i-- ) {
	            		...
	            		pixval[nbPix]=pix;
	            		totalCoef+= pixcoef[nbPix] = getCoef(file.fitsfile,coo,10.);
	            		nbPix++;
	            	}
...
//	            else { empty=false; pixelFinal=totalCoef*1000; }   // Juste pour voir le fading
    ...
    private double getBilinearPixel(Fits f,Coord coo) {
       ...
       if( x1<-2 || x1>f.width+1 || y1<-2 || y1>f.height+1 ) return Double.NaN;
       
       // Pas de bilinéaire sur le bord et 1 pixel de marge
       if( x1<0 || y1<0 || x1>=f.width || y1>=f.height ) {
          if( x1<0 ) x1=0;
          else if( x1>=f.width ) x1=f.width-1;
          if( y1<0 ) y1=0;
          else if( y1>=f.height ) y1=f.height-1;
          double a = getPixelDouble(f,x1,y1);
          if( f.isBlankPixel(a) ) return Double.NaN;
          else return a;
       }
       ...
       double a1 = getPixelDouble(f,x2,y1);
       double a2 = getPixelDouble(f,x1,y2);
       double a3 = getPixelDouble(f,x2,y2);
