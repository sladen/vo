...
//   protected HealpixAllsky [] allsky;// Losanges spéciaux d'accès à tout le ciel niveau 2 et 3
   ...
   protected boolean truePixels=false; // true si le survey est fourni en true pixels (FITS)
...
//      if( !(url.startsWith("http://") || url.startsWith("https://")) ) useCache=false;   // pas de cache pour les fichiers locaux
...
//         System.out.println("PlanBG.setFrameDrawing: => new proj = "+Localisation.REPERE[getFrame()]);
   ...
   private void suite(Coord c,double radius) {
...
//      if( !aladin.NOGUI ) aladin.localisation.setFrame( aladin.configuration.getFrameAllsky() );
      ...
      objet = co+"";
      Projection p = new Projection("allsky",Projection.WCS,co.al,co.del,60*4,60*4,250,250,500,500,0,false,Calib.SIN);
      p.frame = getFrame();
...
//         initZoom = aladin.calque.zoom.getNearestZoomFromRadius(radius+"d");
   ...
   private void initCache() {
      if( cacheSize==-1 ) cacheSize=aladin.configuration.getHpxCacheSize();
      Aladin.trace(3,"Cache Size => "+cacheSize);

      if( Aladin.MAXMEM<200 ) {
         aladin.warning("Full sky Aladin mode requires atleast 200MB of RAM\n" +
   ...
   protected void setPropertie(String prop,String specif,String value) throws Exception {
       if( prop.equalsIgnoreCase("Projection") ) {
           ...
           this.modifyProj(null,Projection.SIMPLE,p.alphai,p.deltai,p.rm1,
                        p.cx,p.cy,p.r1,p.rot,p.sym,Projection.getProjType(value));
           aladin.view.newView(1);
...
//                      System.out.println("je vire le repertoire "+f.getAbsolutePath()+" de taille "+dirSize);
   ...
   protected String key(HealpixKey h) { return key(h.order,h.pixid); }
...
//   }
   ...
   static protected void switchHealpixMode() {
      CURRENTMODE = "Current mode Healpix for query_disc => " + CDSHealpix.switchMode();
   ...
   private boolean tooSmall(ViewSimple v,int order) {
      ...
      double res= CDSHealpix.pixRes((long)nside)/3600;
      double pixSize = v.getPixelSize();
//System.out.println("Resolution pixel order="+order+" => "+Coord.getUnit(res)+" / "+ Coord.getUnit(pixSize));
      return res<pixSize;
   }
   ...
   protected int getAllSkyOrder() {
      if( allSkyOrder==-1 ) {
         if( allsky!=null ) {
            ...
            if( healpix!=null ) {
               allSkyOrder=healpix[0].getOrder();
            }
         }
      }
      return allSkyOrder;
   ...
   protected int getOrder() { 
      ...
      return Math.max(3,Math.min(maxOrder(v),maxOrder));
   }
   ...
   // on conserve en memoire le dernier fichier lu pour répondre plus vite aux demandes de valeurs de flux
   private Fits lastGeneratedFile;
   // mémoire du dernier indice fichier lu
   private long lastIdxGeneratedFile = -1;
   private int lastOrderLosange=-2;
   private long lastHealpixIdxPixel=-1;
   private double lastPixelValue;
   
   protected double getOnePixelFromCache(Projection projd, double x,double y) { return getOnePixelFromCache(projd,x,y,0); }
   protected double getOnePixelFromCache(Projection projd, double x,double y,int order) {

      int orderFile = order<=0 ? getOrder() : order;
      int nSideFile = (int)Math.pow(2,orderFile);
      ...
      try {
         ...
         double ra = coo.al;
         double dec = coo.del;
         if( Double.isNaN(ra) || Double.isNaN(dec) ) return Double.NaN;

         double[] polar = CDSHealpix.radecToPolar(new double[] {ra, dec});

         // lecture directe dans les fichiers générés
         long idxGeneratedFile = CDSHealpix.ang2pix_nest( nSideFile, polar[0], polar[1]);

         if (idxGeneratedFile != lastIdxGeneratedFile || lastGeneratedFile == null) {
            lastIdxGeneratedFile = idxGeneratedFile;

            if( lastGeneratedFile!=null ) lastGeneratedFile.free();
            lastGeneratedFile = new Fits();
            try {
               lastGeneratedFile.loadFITS(getFilePath( getSurveyDir(), orderFile, lastIdxGeneratedFile)+ ".fits");
            } catch (Exception e) {
//               e.printStackTrace();
               if( aladin.levelTrace>=3 ) System.err.println(e.getMessage()+ " pixel "+x+","+y+" coo="+coo+" order="+orderFile+" projd="+projd+") => Double.NaN");
               lastIdxGeneratedFile = -1;
               lastGeneratedFile = null;
               return Double.NaN;
            }
         }
         
         int nSideLosange = lastGeneratedFile.width;
         int orderLosange = (int)PixTools.log2(nSideLosange);
         
         long healpixIdxPixel = CDSHealpix.ang2pix_nest(nSideFile*nSideLosange, polar[0], polar[1]);
         if( lastHealpixIdxPixel==healpixIdxPixel ) return lastPixelValue;
         
         long startIdx = (long) idxGeneratedFile * (long)Math.pow(4, orderLosange);
         if (hpx2xy == null || orderLosange!=lastOrderLosange)  {
            createHealpixOrder(orderLosange);
            lastOrderLosange=orderLosange;
         }
         int idx = hpx2xy((int)(healpixIdxPixel-startIdx));
         int yy = idx/nSideLosange;
         int xx = idx-yy*nSideLosange;
         
         double pixel = lastGeneratedFile.getPixelFull(xx, yy);
         lastPixelValue = pixel;
         lastHealpixIdxPixel=healpixIdxPixel;
         
//         System.out.println(orderFile+"/"+nSideFile+
//               " Losange: order="+orderLosange+" nsidelosange="+nSideLosange+" idxGeneratedFile="+idxGeneratedFile+
//               " healpixIdxPixel="+healpixIdxPixel+" idx="+idx+" x="+xx+" y="+yy+" pixel="+pixel);
...
//         System.err.println(e.getMessage());
   ...
   protected int maxOrder(ViewSimple v) {
      ...
      if( RES==null ) {
         ...
         for( order=0; order<20; order++ ) {
            double nside = Math.pow(2,order+getLosangeOrder()/*+1*/);
            RES[order]=CDSHealpix.pixRes((long)nside)/3600;
         }
      }
      double pixSize = v.getPixelSize();
      for( order=2; RES[order]>pixSize; order++ );
      if( order==2 && pixSize<0.04 ) order=3;
      return order;
   }
   ...
   public double getPixelResolution() {
      long nside = (long) Math.pow(2,getMaxHealpixOrder());
...
//   protected boolean isDrawn() { return pixList.size()>0 && hasDrawnSomething; }
   ...
   protected boolean isSync() {
      boolean rep = error!=null || flagOk && (!active || getOpacityLevel()==0f || !flagProcessing && !isLoading() && isDrawn());
//      if( !rep ) aladin.trace(4,"PlanBG.isSync()=false => thread="+this.hashCode()+" "+label+" isSync="+rep+" error="+(error==null?"null":error)+
//            " hasDrawSomeThing="+hasDrawnSomething+" pixList="+(pixList!=null ? pixList.size()+"":"")+
//            " pourcent="+pourcent+ " readyDone="+readyDone+" readyAfterDrawn="+readyAfterDraw);
      return rep;
   }
   ...
   private boolean childrenReady(HealpixKey healpix,ViewSimple v) {
      ...
      for( int i=0; i<4; i++ ){
         ...
         if( fils==null ) fils = new HealpixKey(this,order,children[i],true);
...
//System.out.println("***** "+ healpix+" inutile car a tout ses fils");
   ...
   protected void modifyProj(String label,int modeCalib,
      ...
      super.modifyProj(label,modeCalib,alphai,deltai,rm,cx,cy,r,rot,sym,t);
...
//      }
   ...
   protected void drawCtrl(Graphics g,ViewSimple v,int order) {
      try {
         ...
         for( int i=0; i<pix.length; i++ ) {
            HealpixKey healpix = new HealpixKey(this,order,pix[i],true);
   ...
   protected void setDebugIn(double raj,double dej,double radius,int order) {
      if( !DEBUGMODE ) return;
      long [] npix = null;
      ...
      if( diff ) {
         ...
         System.out.print("Location: "+aladin.localisation.J2000ToString(raj,dej)+" Radius: "+Coord.getUnit(radius)+" Nside=2^"+order+" => ");
...
//   }
   ...
   protected byte [] getCurrentBufPixels(Plan pi,RectangleD rcrop, double zoom) {
      lockGetPixelInfo=true;
      int w = ViewSimple.top(rcrop.width*zoom);
      int h = ViewSimple.top(rcrop.height*zoom);
      ...
      for( int y=h-1; y>=0; y-- ) {
         pi.pourcent+=fct;
         for( int x=0; x<w; x++ ) {
            double val = getOnePixelFromCache(pi.projd,rcrop.x + (x+0.5)/zoom,rcrop.y + (y+0.5)/zoom,order);
            if( Double.isNaN(val) ) {
...
//   private double getBilinearPixel(Projection proj,double x, double y,int order) {
//      double x1=(int)x-0.5,x2=(int)x1+1;
//      double y1=(int)y-0.5,y2=(int)y1+1;
//
//      double a0 = getOnePixelFromCache(proj,x1,y1,order);
//      double a1 = getOnePixelFromCache(proj,x2,y1,order);
//      double a2 = getOnePixelFromCache(proj,x1,y2,order);
//      double a3 = getOnePixelFromCache(proj,x2,y2,order);
//
...
//   }
   ...
   protected void drawHealpixMouse(Graphics g,ViewSimple v) {
      try {
         ...
         HealpixKey hk = new HealpixKey(this,norder,npix,true);
         hk.drawCtrl(g, v);
      } catch( Exception e ) {
         e.printStackTrace();
      }
   ...
   protected void resetPriority() {
      ...
      while( e.hasMoreElements() ) {
         HealpixKey healpix = e.nextElement();
         if( healpix.pixid!=-1 && healpix.priority<1000 ) healpix.priority+=1000;
      }
      priority=0;
   }
...
//   }
   ...
   protected void drawPolarisation(Graphics g,ViewSimple v) {
      ...
      for( int i=0; i<pix.length; i++ ) {
         ...
         if( (new HealpixKey(this,order,pix[i],true)).isOutView(v) ) continue;
...
//      System.out.println("Order="+max+" v="+v.rv.width+"x"+v.rv.height);
      ...
      boolean fast = aladin.view.mustDrawFast();
      if( fast ) min=max;
...
//          On place le losange central en premier dans la liste
         try {
            if( center!=null ) {
               long firstPix = CDSHealpix.ang2pix_nest((int)Math.pow(2,order),theta, phi);
         ...
         for( int i=0; i<pix.length; i++ ) {
            ...
            if( (new HealpixKey(this,order,pix[i],true)).isOutView(v) ) {
               nOut1++;
               pix[i]=-1;
               continue;
            }
            ...
            nb+=healpix.draw(g,v);
//            s1.append(" "+healpix.getStringNumber());
...
//            setBufPixels8(healpix.pixels);   // Pour que l'histogramme soit à jour
      ...
      statNbItems = nb;
//System.out.println("Draw["+min+".."+max+"] "+s1+" "+
//      +nb+" losanges in "+(statTimeDisplay)+"ms");
   }


   private int redraw(Graphics g,ViewSimple v) {
      int n=0;
      if( redraw.size()==0 ) return 0;
...
//      if( rayon<60 ) return;
      ...
      g.setStroke(new BasicStroke(EP));
      int m =rayon<100 ? 0 : rayon<300 ? M/2 : M;
      if( projd.t==Calib.SIN || projd.t==Calib.ARC || projd.t==Calib.ZEA) {
         g.drawOval(x-(EP/2)+m,y-(EP/2)+m,(rayon-m)*2+EP,(rayon-m)*2+EP);
      } else if( projd.t==Calib.AIT || projd.t==Calib.MOL) {
         g.drawOval(x-(EP/2)+m,y-(EP/2)+m,(grandAxe-m)*2+EP,(rayon-m)*2+EP);
      }
      g.setStroke(st);
...
//System.out.println(label+" running...");
               ...
               try {
                  ...
                  if( minHealpix!=null ) {
                     ...
                     if( !minHealpix.allSky ) setLosangeOrder(minHealpix.getOrder());
...
//                     System.out.println(label+" wakeup !");
   ...
   protected String getShortStats() { 
      ...
      return label+(!url.startsWith("http:")?" Local:":" Net:")+nbLoadNet  +"/"+Util.round(nByteReadNet/(1024*1024.),1)  +"Mb/" +Util.round(rateLoadNet(),1)  +"ms"
      +" CacheR:"+nbLoadCache+"/"+Util.round(nByteReadCache/(1024*1024.),1)+"Mb/" +Util.round(rateLoadCache(),1)+"ms"
      +" CacheW:"+nbWriteCache+"/"+Util.round(nByteWriteCache/(1024*1024.),1)+"Mb/" +Util.round(rateWriteCache(),1)+"ms";
   }
