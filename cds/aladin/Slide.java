...
//                  xPoignee=fillTransparency(g,dx,dy,transp,p.active || p.isImage() ); 
         ...
         } else if( p.type==Plan.IMAGEBLINK || p.type==Plan.IMAGECUBE ) {
         ...
         if( mode!=DRAG && canBeTransparent ) {
            if( xPoignee<dx+frX[1]+3 ) xPoignee = dx+frX[1]+3;
            if( xPoignee>dx+frX[2]-3 ) xPoignee = dx+frX[2]-3;
//            g.setColor( Color.magenta.darker() );
            g.setColor( Color.black );
            g.drawLine( dx+frX[1],dy+frY[1],dx+frX[2],dy+frY[2]);
            g.drawLine( dx+frX[1],dy+frY[1]+1,dx+frX[2],dy+frY[2]+1);
            g.drawRect(xPoignee-1,dy+frY[1]-3,2,6);
            g.setColor(Color.white);
            g.drawLine(xPoignee,dy+frY[1]-2,xPoignee,dy+frY[1]+1);
         }
         ...
         if( p.type!=Plan.NO && mode!=DRAG && p.flagOk && !p.hasError()
            ...
            MCanvas.drawCheckbox(g, 3, dy+3, Color.gray ,
