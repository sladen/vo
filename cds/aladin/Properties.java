...
//      INFO = aladin.chaine.getString("PROPINFO");
...
//      TOPBOTTOM = aladin.chaine.getString("PROPTOPBOTTOM");
//      RIGHTLEFT = aladin.chaine.getString("PROPRIGHTLEFT");
//      NEWCOL = aladin.chaine.getString("PROPNEWCOL");
...
//      MyLabel l = new MyLabel(info,JLabel.CENTER,Aladin.SITALIC);
   ...
   * @param c      Les contraintes courantes sur le gestionnaire d'affichage
   */
   protected static void addCouple(final JFrame frame, JPanel p, Object titre, final String help, Component valeur,
...
//      if( plan.type==Plan.IMAGE || plan.type==Plan.IMAGERGB && ((PlanImage)plan).headerFits!=null ) {
      if( plan.isImage() || plan.type==Plan.ALLSKYIMG) {
        ...
        String survey=pimg.survey();
      	...
      	// Format d'image
        JLabel fmtl = new JLabel(PlanImage.describeFmtRes(pimg.dis,pimg.res));
      ...
      // Origine
      if( plan.from!=null ) addCouple(p,ORIGIN, new JLabel(plan.from), g,c);
...
//         if( plan.getUrl()!=null && (plan.type==Plan.CATALOG || plan.type==Plan.FOLDER) ) {
//            panelInfo.add(b=new JButton(SEEPARSING));
//            b.addActionListener(this);
//         }
         c.fill = GridBagConstraints.NONE;
      ...
      if( plan.type==Plan.ALLSKYIMG ) {
         ...
         addFilet(p, g, c);
         addSectionTitle(p, "Tesselation properties", g, c);
         addCouple(p, "Format", new JLabel(pbg.getFormat()), g, c);
         addCouple(p, "Best pixel resolution", 
               new JLabel(pbg.getMaxResolution()+" (HEALpix nside=2^"+pbg.getMaxHealpixOrder()+")"), g, c);
         addCouple(p, "Rhomb file width:", 
               new JLabel("2^"+pbg.getLosangeOrder()+"="+(int)Math.pow(2,pbg.getLosangeOrder())+"pix"), g, c);
