...
public class HealpixKeyCat extends HealpixKey {
   ...
   protected void updateCacheIfRequired(int time) throws Exception {
      ...
      try {
         ...
         stream = readFully(dis);
         System.out.println(getStringNumber()+" => cache update");
         writeCache();
         dis.close();
      } catch( Exception e ) {
   ...
   protected boolean isReallyLast(ViewSimple v) {
      if( last ) return true;
      for( int i=0; i<4; i++ ) {
         long filsPixid = pixid*4+i;
         if( (new HealpixKey(planBG, order+1, filsPixid, true)).isOutView(v) ) continue;
